...
void process_digit(int digit, int position, int scale){
    for(int i=0; i<7; i++){
...
void process_digit_forward_slanted(int digit, int position,int scale){
    //space the digit out base on their scale when slant
    if(scale == 2){space_between_digit = 5;}
    if(scale == 3){space_between_digit = 6;}
    if(scale == 4){space_between_digit = 8;}
    if(scale == 5){space_between_digit = 9;}
...
void process_digit_back_slanted(int digit, int position,int scale){
    //space the digit out base on their scale when slant
    if(scale == 2){space_between_digit = 5;}
    if(scale == 3){space_between_digit = 6;}
    if(scale == 4){space_between_digit = 8;}
    ...
    for(int i=0;i<7;i++){
        if(truthTable[digit][i]==1){
            switch(i){         
                ...
                case 1:// middle zone left region
                if (scale==5){       
                digits[(scale-4)][(scale-1)-4 +position*space_between_digit]='\\';         
                digits[(scale-3)][(scale-2)-4 +position*space_between_digit]='\\';
                digits[(scale-2)][(scale-3)-4 +position*space_between_digit]='\\';
                digits[(scale-1)][(scale-4)+position*space_between_digit]='\\';
                digits[(scale)][(scale-5)-4+position*space_between_digit]='\\';
                }
                if (scale==4){
                digits[(scale-3)][(scale)-4 +position*space_between_digit]='\\';
                digits[(scale-2)][(scale-1)-4 +position*space_between_digit]='\\';
                digits[(scale-1)][(scale-2)-4+position*space_between_digit]='\\';
                digits[(scale)][(scale-3)-4+position*space_between_digit]='\\';
                }
                if (scale==3){
                digits[(scale-2)][(scale)-4 +position*space_between_digit]='\\';
                digits[(scale-1)][(scale-1)-4+position*space_between_digit]='\\';
                digits[(scale)][(scale-2)-4+position*space_between_digit]='\\';
                }
                if (scale==2){
                digits[(scale-1)][(scale+1)-4+position*space_between_digit]='\\';
                digits[(scale)][(scale)-4+position*space_between_digit]='\\';
                }
                break;
                ...
                case 6:// bottom  zone center region 
                if (scale==5){
                digits[(scale+scale)][(scale-9)+8+position*space_between_digit]='_';
                digits[(scale+scale)][(scale-8)+8+position*space_between_digit]='_';
                digits[(scale+scale)][(scale-7)+8+position*space_between_digit]='_';
                ...
                digits[(scale+scale)][(scale-4)+8+position*space_between_digit]='_';
                digits[(scale+scale)][(scale-3)+8+position*space_between_digit]='_';
                digits[(scale+scale)][(scale-2)+8+position*space_between_digit]='_';
                }
                if (scale==2){
                digits[(scale+scale)][(scale-1)+8+position*space_between_digit]='_';
                digits[(scale+scale)][(scale)+8+position*space_between_digit]='_';
                }
                break;
            }
...
void read_input (char *input, int scale, char *style){
    int number = 0;
    int position = 0;
    ...
    for(int i = 0; i < inputLen; i++){
        ...
        if(position<inputLen){
            if (scale >= 1 && scale <=5){
                if(style[0] == 'n'){
                    process_digit(number,position, scale);
                }else if (style[0] == 'f'){
                    process_digit_forward_slanted(number,position, scale);
                }else if (style[0] == 'b'){
                    process_digit_back_slanted(number,position, scale);
                }else{
