...
//      int scale: scale of the number being load 
//      char *style: style of the number being load
void display_all_zone (char *input, int scale, char *style){
...
int main(int argc, char *argv[]) {
   ...
   // turn argv[2] scaler into integer scale
    ...
    display_all_zone(input, 1, "n");
        ...
        if (scale >= 1 && scale <=5){// scale have to be between 1 and 5
            display_all_zone(input, scale, "n");
        }else{
        ...
        if (scale >= 1 && scale <=5){// scale have to be between 1 and 5
            if (style[0] == 'n' || style[0] == 'f'|| style[0] == 'b'){// only allow n(normal), f(forward),b(back) to be use as style
                display_all_zone(input, scale, style);
            }else {
