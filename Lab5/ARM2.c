...
int registerCtoI(char *registerChar){
   ...
   returnVal = strtol(tempChar, &pointer, 10);
   return returnVal;
...
int locateSource(char *src, char *val){
   ...
   hold2 = strtol(location, &pointer, 10);
...
void LDUR(char *dest,char *src, char *val){
   ...
   if(strcmp(src, "[SP") == 0){
      returnVal = srcInt;
      registers[destInt] = strtol(memory[returnVal][1], &pointer, 10); 
   }else{
      returnVal = (srcInt - 100)/4;
      registers[destInt] = strtol(memory[returnVal][1], &pointer, 10);
   }
...
void CBNZ(char *dest, char *src){
   ...
   if(registers[destInt] != 0){
      branchDest = strtol(src, &pointer, 10);
      pCounter = branchDest;
   }
}
...
void BL(char *dest) {
    ...
    progDest = strtol(dest, &pointer, 10);
    pCounter = progDest;
}
...
int main(int argc, char* argv[]) 
{
   ...
   if(argc == 2){
      while(fgets(buff, 50, file)) 
      {
         ...
         memoryLocation = strtol(progCounter, &pointer, 10);
